...
# Performance UX Optimization: Added -c (continue/resume) and -q --show-progress to prevent freezing and save network bandwidth
wget -c -q --show-progress "https://cdimage.ubuntu.com/ubuntu-base/releases/${UBUNTU_VERSION}/release/ubuntu-base-${UBUNTU_VERSION}-base-${ARCHITECTURE}.tar.gz" -O ubuntu.tar.gz
